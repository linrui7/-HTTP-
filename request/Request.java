...
public class Request {
    ...
    public static Request parse(InputStream inputStream) throws IOException {
        ...
        //设置请求头参数
        parseRequestHeader(bufferedReader, request);
        //如果是doPost方法有请求体
        if (request.method.toUpperCase().equals("POST")) {
            parseRequestBody(bufferedReader, request);
        }
        return request;
    }
    ...
    private static void parseRequestLine(BufferedReader bufferedReader, Request request) throws IOException {
    ...
    // Host：www.baidu.com
    // Accept:text/html
    private static void parseRequestHeader(BufferedReader bufferedReader, Request request) throws IOException {
        String secondLine;
    ...
    //第三部分 请求体
    // 如果是doPost请求体里面包含很多东西，比如表单提交。
    //rg:id=1&password=2
    private static void parseRequestBody(BufferedReader bufferedReader, Request request) throws IOException {
        int len = Integer.parseInt(request.headers.get("Content-Length"));
        char[] buf = new char[len];
        bufferedReader.read(buf, 0, len);
        String frag = String.valueOf(buf);
        String[] mess = frag.split("&");
        //将请求体里面的参数也加进去
        for (int i = 0; i < mess.length; i++) {
            String[] keyvalue = mess[i].split("=");
            String key = keyvalue[0];
